...
to go
  ...
  update-turtles
  update-globals
  if the-turn >= ticks [ move-unhappy-turtles
  tick
  ]
end
...
to check-found-bugs [ msg ]
  ...
  let spot patch clicked-xcor clicked-ycor

  if not any? other turtles-on spot
  [move-to spot
  set a-turn a-turn + 1]
  update-happy?-monitor

end


to move-unhappy-turtles
  ask turtles with [ not happy? and breed != users]
  [ find-new-spot ]
  set t t + 1
end
...
to find-new-spot
  ...
  let empty-patches patches in-radius 8 with [not any? turtles-here]
  ifelse any? empty-patches
  [let new-home one-of empty-patches
  ;  ask empty-patches [set pcolor pcolor - 0.05]
    ask new-home [set d distance myself
...
to update-turtles
  ask turtles [
    ...
    set happy-red ( count turtles with [ color = red and happy? = TRUE ] ) / (count turtles with [color = red])
    set happy-green ( count turtles with [ color = green and happy? = TRUE ] ) / (count turtles with [color = green])
  ]
end
...
to update-globals
  ...
  if any? users [ set the-turn sum [a-turn] of users / (count users) ]
end

to update-happy?-monitor
  ;ifelse show-sick-on-clients?
  ;[ hubnet-send user-id "Sick?" infected? ]
  ;[ hubnet-send user-id "Sick?" "N/A" ]
  hubnet-send user-id "happy?" happy?
  hubnet-send user-id "happy-red" happy-red
  hubnet-send user-id "happy-green" happy-green
end
