...
to go
  ...
  make-node find-partner         ;; find partner & use it as attachment
                                 ;; point for new node
  if ticks mod 20 = 0 [ remove-nodes ]
...
to make-node [old-node]
  create-turtles 1
  [
    set color (item (random number-of-groups) colors)
    set want-links number-of-links
    set me self
    if old-node != nobody
    [
      ifelse count turtles <= 5
      [
      ...
      [create-links old-node]
    ]
  ]
end

to create-links [old-node]
  if want-links > 0
  [
    create-new-link find-partner
    set want-links want-links - 1
  ]
end

to create-new-link [old-node]
  if old-node != me [
    ifelse [color] of old-node  = [color] of me [
      create-link-with old-node [ set color green ]
      ;; position the new node near its partner
      move-to old-node
      fd 8
