...
to setup
  ...
  set-default-shape turtles "circle"
   set colors [red green yellow blue orange pink white ]
  ;; make the initial network of two turtles and an edge
  create-turtles number-of-links + 1 [
    set color (item (random number-of-colors) colors)
  ]
  ask n-of number-of-links turtles [ create-link-with one-of other turtles
    fd 3]
  reset-ticks
end
...
to make-node
  ...
  [
    set color (item (random number-of-colors) colors)
    set want-links number-of-links
  ]
end
...
to make-link
  ...
  ask turtles [
    if want-links >= 1 [
      ifelse random 100 < homophily-level [
        ;let me self
        ;let color-of-me [color] of self
        ;let target [one-of both-ends with [color = [color] of myself]] of one-of links
        ask other turtles with [color = [color] of myself] [
          repeat degree [ set targets lput self targets ]
        ]
        if targets != [] [
        ...
        let target [one-of both-ends] of one-of links
        if target != nobody and target != self
        [ create-link-with target [ set color green ]
