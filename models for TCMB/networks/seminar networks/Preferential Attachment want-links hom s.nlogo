...
to make-node
  ...
  [
    ...
    set want-links one-of [ 1 1 1 1 2 3 4 ] ;number-of-links
  ]
end


to make-link
  ask turtles with [count link-neighbors < want-links] [
    let colornow [color] of self

    ifelse random 100 < h-level [

      let target [one-of both-ends with [color = colornow]] of one-of links
