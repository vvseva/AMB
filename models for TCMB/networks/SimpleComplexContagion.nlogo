...
to spread
  let colortospread white
  ;; infection can't take place if every agent has already been infected
  ifelse not any? turtles with [color = white]
    [stop]
   
[
  ask turtles with [ color != white ]
  [
    set colortospread color
    ;; infect neighbors
    ask link-neighbors
    [ 
      if ( random-float 1 <= prob-infection )  ;; infect with probability prob-infection
      [
        if (color = white)  ;; agents can be infected only once
        [
          set color colortospread
          ask link-with myself [set color (colortospread + 2)]
          show-turtle
        ]
      ]
    ]
...
;; in complex contagion, a node will only adopt an opinion
;; if at least two of its neighbors share that opinion
;; it will decide based on the proportion of its neighbors
;; that are either blue or red
to complex-spread
  let colortospread white
  let probblueoverred 0
  ifelse not any? turtles with [color = white] 
    [stop]   
[
  ask turtles with [ color = white ]
  [
    let redneighbors count link-neighbors with [color = red]
    let blueneighbors count link-neighbors with [color = blue]
    ifelse (redneighbors >= 2)
    [ 
      ifelse (blueneighbors < 2) 
      [
          set color red
          show-turtle
      ][
         set probblueoverred (blueneighbors) / (count link-neighbors)
         ifelse ( random-float 1 <= probblueoverred ) [
           set color blue
         ][
           set color red
         ]
