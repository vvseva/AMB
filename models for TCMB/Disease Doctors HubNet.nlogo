...
to go
  ...
  [
    ...
    ask students [ suffer update-globals]
...
to suffer
  ...
  ifelse any? other turtles-here with [breed = students] [
    ask turtles-here with [breed = students] [set happy? 20]
  ]
  [
    set happy? happy? - 0.1
  ]
...
to update-sick?-monitor ;; turtle procedure
  set total-infected (count students with [ infected? = TRUE ]) / ( count students ) * 100
  if show-infected-num [ hubnet-send user-id "Infected?" total-infected ]
...
to update-globals
  ...
  set percent-happy ( sum [happy?] of students )  / (count students)
  if any? students [hubnet-send user-id "happy" happy?]
end
