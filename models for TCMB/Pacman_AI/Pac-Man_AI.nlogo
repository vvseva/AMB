...
to move-pacman  ;; Observer Procedure
  ...
  [ ;; move forward unless blocked by wall
    if AI [ act-independent ] ; AI featuries
...
to act-independent
  let old-heading heading
  set-pref
  if scared < 2 ; to act brave
  [ if any? ghosts-on patch-ahead 1
    or any? ghosts-on patch-right-and-ahead 90 1
    or any? ghosts-on patch-right-and-ahead -90 1 [set new-heading old-heading + 180] ]
  if [pcolor] of patch-right-and-ahead 90 1 = black
  and [pcolor] of patch-right-and-ahead -90 1 = black
  and random 100 - pref-ahead > 50
  [
    ask pacmans [ set new-heading old-heading + one-of [90 -90] ]
    stop
  ]

  if [pcolor] of patch-right-and-ahead 90 1 = black and random 100 - pref-ahead + pref-right > 50 [
    ask pacmans [ set new-heading old-heading + 90 ]
    stop
  ]

  if [pcolor] of patch-right-and-ahead -90 1 = black and random 100 - pref-ahead + pref-left > 50 [
    ask pacmans [ set new-heading old-heading - 90 ]
    stop
  ]
  if [pcolor] of patch-ahead 1 != black [act-independent]
;  if [pcolor] of patch-ahead 1 != black [
...
to set-pref
  ...
  set pref-left 0
  set pref-right 0
  if any? pellets-on patch-ahead 1 [set pref-ahead 40]
  if any? pellets-on patch-right-and-ahead -90 1 [set pref-left 40]
  if any? pellets-on patch-right-and-ahead 90 1 [set pref-right 40]
end
